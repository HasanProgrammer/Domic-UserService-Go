...
func NewEvent[TIdentity any](id string, name string, table string, action string, payload string, createdAt time.Time, createdBy TIdentity, createdRole string) *Event[TIdentity] {
	return &Event[TIdentity]{id: id, name: name, table: table, action: action, payload: payload, createdAt: createdAt, createdBy: createdBy, createdRole: createdRole}
}
